<div class="box box-info padding-1">
    <div class="box-body">
        ...
        <input type="hidden" name="detalleCounter" id="detalleCounter"
        ...
        @endphp
        <div id="detail-copy" class="{{ $areThere ? 'detail-copy-form' : '' }}">
            <!-- Oculta el contenido en caso de haber registros -->
            ...
            <div class="row detail-principal">
                <div class="col-12">
                    ...
                    <div class="row">
                        <div class="col-10">
                            <h3 class="text-center fs-4 fw-bold">DETALLES DEL VUELO <strong
                                    id="number_{{ $numberAreThere }}">#1</strong></h3>
                        </div>
                        <div class="col-2">
                ...
                {{-- Termina columna 12 --}}
                <input name="id_detail_operation_{{ $numberAreThere }}" id="id_detail_operation_{{ $numberAreThere }}"
            ...
            @foreach ($operation->details as $key => $detail)
                <div class="row">
                    <div class="col-12">
                        <br>
                        <h3 class="text-center fs-4 fw-bold">DETALLES DEL VUELO <strong
                                id="number_{{ $key + 1 }}">#{{ $key + 1 }}</strong></h3>
                    </div>
                    {{-- Termina columna 12 --}}
                    <input name="id_detail_operation_{{ $key + 1 }}"
                        id="id_detail_operation_{{ $key + 1 }}" type="text" value="{{ $detail->id }}"
                        hidden>
                    <div class="col-12 col-md-12">
                        <div class="form-group">
                    ...
                    <div class="col-12 col-md-8 section-evidence">
                        <div class="form-group">
                            <input type="text" name="files_evidence_delete_{{ $key + 1 }}"
                                id="files_evidence_delete_{{ $key + 1 }}" value="" hidden>
                            {{ Form::label('files_' . $key + 1, 'Subir Evidencias (Máximo 7)') }}
                            <section id="multi-selector-uniq">
                                <input class="form-control" id="files_{{ $key + 1 }}"
                                    name="files_{{ $key + 1 }}[]" type="file" multiple accept="image/*">
                                ...
                                <ul id="preview-files-load">
                                    @forelse ($detail->files_details as $files_evidence)
                                        <li class="section-evidence-preview">
                                            <img src="{{ asset($files_evidence->src_file) }}" width="80%"
                                                height="100px">
                                            <button class="btn btn-danger btn-preview-image" type="button"
                                                data-key="{{ $key + 1 }}"
                                                data-src="{{ $files_evidence->src_file }}">X</button>
                                        </li>
                                    @empty
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            $(".duplicarDetalleOperacion").click(function() {
                ...
                var nameScriptFile = 'js/views/previews/multiple.image_' + keyCounter +
                '.js'; // Name file siguiente
