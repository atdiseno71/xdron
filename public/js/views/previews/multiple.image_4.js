...
const fileInputMulti_4 = document.querySelector("#multi-selector-uniq #files_4");
// DEMO Preview
const multiSelectorUniqPreview_4 = document.querySelector("#multi-selector-uniq #preview-files_4");
...
function renderPreviews(currentFileList, target, inputFile) {
    ...
    currentFileList.forEach((file, index) => {
        ...
        myButtonRemove.addEventListener("click", () => {
            filesList_4 = deleteArrayElementByIndex(currentFileList, index);
            inputFile.files = arrayFilesToFileList(filesList_4);
            return renderPreviews(filesList_4, multiSelectorUniqPreview_4, inputFile);
        });
 ...
 * @param {Array<File>} filesList_4
 * @return {FileList}
 */
function arrayFilesToFileList(filesList_4) {
    return filesList_4.reduce(function (dataTransfer, file) {
...
// Input file
fileInputMulti_4.addEventListener("input", async () => {
    // Get files list from <input>
    const newFilesList = Array.from(fileInputMulti_4.files);
    // Update list files
    filesList_4 = await getUniqFiles(newFilesList, filesList_4);
    // Only DEMO. Redraw
    renderPreviews(filesList_4, multiSelectorUniqPreview_4, fileInputMulti_4);
    // Set data to input
    fileInputMulti_4.files = arrayFilesToFileList(filesList_4);
});

// Drag and drop

// Drag Start - Moving element.
let myDragElement_4 = undefined;
document.addEventListener("dragstart", (event) => {
    // Saves which element is moving.
    myDragElement_4 = event.target;
});
...
function eventDrop(event) {
    ...
    filesList_4 = arraySwapIndex(
        getIndexOfFileList(myDragElement_4.dataset.key, filesList_4),
        getIndexOfFileList(myDropElement.dataset.key, filesList_4),
        filesList_4
    );
