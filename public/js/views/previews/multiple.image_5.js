...
const fileInputMulti_5 = document.querySelector("#multi-selector-uniq #files_5");
// DEMO Preview
const multiSelectorUniqPreview_5 = document.querySelector("#multi-selector-uniq #preview-files_5");
...
function renderPreviews(currentFileList, target, inputFile) {
    ...
    currentFileList.forEach((file, index) => {
        ...
        myButtonRemove.addEventListener("click", () => {
            filesList_5 = deleteArrayElementByIndex(currentFileList, index);
            inputFile.files = arrayFilesToFileList(filesList_5);
            return renderPreviews(filesList_5, multiSelectorUniqPreview_5, inputFile);
        });
 ...
 * @param {Array<File>} filesList_5
 * @return {FileList}
 */
function arrayFilesToFileList(filesList_5) {
    return filesList_5.reduce(function (dataTransfer, file) {
...
// Input file
fileInputMulti_5.addEventListener("input", async () => {
    // Get files list from <input>
    const newFilesList = Array.from(fileInputMulti_5.files);
    // Update list files
    filesList_5 = await getUniqFiles(newFilesList, filesList_5);
    // Only DEMO. Redraw
    renderPreviews(filesList_5, multiSelectorUniqPreview_5, fileInputMulti_5);
    // Set data to input
    fileInputMulti_5.files = arrayFilesToFileList(filesList_5);
});

// Drag and drop

// Drag Start - Moving element.
let myDragElement_5 = undefined;
document.addEventListener("dragstart", (event) => {
    // Saves which element is moving.
    myDragElement_5 = event.target;
});
...
function eventDrop(event) {
    ...
    filesList_5 = arraySwapIndex(
        getIndexOfFileList(myDragElement_5.dataset.key, filesList_5),
        getIndexOfFileList(myDropElement.dataset.key, filesList_5),
        filesList_5
    );
